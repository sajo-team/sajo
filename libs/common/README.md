...
- **공통 에러 처리**: 호출한 서비스가 우리 `ErrorResponse` 포맷(`errorCode`, `message`)으로 에러를 내려주면, 그걸 파싱해서 호출한 쪽에도 그대로 `BusinessException`을 던짐 — `GlobalExceptionHandler`가 알아서 잡아서 원래 에러코드/메시지 그대로 응답해줌. 우리 포맷이 아닌 응답(파싱 실패)이면 `ErrorResponseCode.FEIGN_CALL_FAILED`(`COMMON_9998`)로 대체됨.
